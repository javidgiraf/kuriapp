...
class ProfileController extends Controller
{
  ...
  public function userSubscriptions()
  {
    ...
    $user_subscriptions = UserSubscription::with(['deposits.deposit_periods', 'scheme.schemeType', 'scheme.schemeSetting'])
      ...
      ->map(function ($subscription) {
        ...
        $dueDate = $schemeSetting ? now()->startOfMonth()->addDays($scheme->schemeSetting->due_duration - 1) : null;
        ...
        return [
          ...
          'title' => trans("messages.scheme_title_" . $scheme->scheme_type_id),
  ...
  public function current_plan_history($scheme_id)
  {
    ...
    $user_subscription = UserSubscription::with(['deposits.deposit_periods', 'scheme', 'schemeSetting', 'scheme.schemeType'])
      ->where('user_id', $id)
      ->where('scheme_id', $scheme_id)
      ->where('is_closed', false)
      ->first();
    ...
    if ($user_subscription) {
      ...
      $balance_amount = collect($user_subscription_deposits)->map(function ($deposit) use ($user_subscription) {
        $totalSchemeAmount = $deposit->deposit_periods->sum('scheme_amount');
        ...
        return ($user_subscription->subscribe_amount * $monthsCount) - $totalSchemeAmount;
      })->first();
  ...
  public function deposit(Request $request, $sub_id)
  {
    ...
    // Validate maximum allowed amount after six months
  ...
  {
    $id = auth()->user()->id;
    $user_subscriptions = UserSubscription::with('scheme', 'deposits')->where('user_id', $id)->get();
    $schemeType = $user_subscriptions->first()->scheme->scheme_type_id;
    $sum = 0;
    ...
    if ($schemeType == SchemeType::GOLD_PLAN) {
      $latestGoldRate = GoldRate::select('date_on', 'per_gram', 'per_pavan')->latest('date_on')->first();
  ...
  public function completedSubscriptions()
  {
    $userSubscriptions = UserSubscription::with(['scheme' => function ($query) {
      ...
      }])->select('id', 'title', 'scheme_type_id');
    }])
    ...
    $flattenedSubscriptions = $userSubscriptions->map(function ($subscription) {
      return [
        ...
        'is_closed' => $subscription->is_closed,
        'status' => $subscription->status,
        'scheme_title' => $subscription->scheme->title ?? null,
